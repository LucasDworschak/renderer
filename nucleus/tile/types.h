...
struct GpuVectorLayerTile {
    ...
    std::shared_ptr<const nucleus::Raster<uint32_t>> grid_triangle;
    // 24 bits start_index, 8 bits size
    std::vector<std::shared_ptr<const nucleus::Raster<uint32_t>>> grid_lines;
    ...
    std::vector<std::shared_ptr<const nucleus::Raster<uint32_t>>> grid_to_data;
    // vertex 0, 1, 2 position + style_index
    std::vector<std::shared_ptr<const nucleus::Raster<uint32_t>>> data_triangle;
    // vertex 0, 1 position + style_index
    std::shared_ptr<const std::vector<const nucleus::Raster<uint32_t>>> data_lines;
};

static_assert(NamedTile<GpuVectorLayerTile>);

struct GpuVectorLayerQuad {
    tile::Id id;
    std::array<GpuVectorLayerTile, 4> tiles;
};

static_assert(NamedTile<GpuVectorLayerQuad>);
